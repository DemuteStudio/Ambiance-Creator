...
-- Function to place items for a container
function Generation.placeItemsForContainer(container, containerTrack, xfadeshape)
  if container.items and #container.items > 0 then
    if container.useRepetition then
      -- Placement based on trigger rate with drift
      local lastItemEnd = globals.startTime -- Start at the beginning of time selection
      ...
      while lastItemEnd < globals.endTime do
        ...
        local randomItemIndex = math.random(1, #container.items)
        local itemData = container.items[randomItemIndex]
        
        -- Calculate interval based on the selected mode
        local interval = container.triggerRate -- Default (Absolute mode)
        
        if container.intervalMode == 1 then
          -- Relative mode: Interval is a percentage of time selection length
          interval = (globals.timeSelectionLength * container.triggerRate) / 100
        elseif container.intervalMode == 2 then
          ...
          local itemCount = #container.items
          ...
          if itemCount > 0 then
            ...
            local desiredCoverage = container.triggerRate / 100 -- Convert percentage to ratio
        ...
        local position
        if container.intervalMode == 0 and interval < 0 then
          -- Negative spacing creates overlap with the last item (only applicable in Absolute mode)
          local maxDrift = math.abs(interval) * (container.triggerDrift / 100)
          ...
          local maxDrift = interval * (container.triggerDrift / 100)
          local drift = Utils.randomInRange(-maxDrift/2, maxDrift/2)
          position = lastItemEnd + interval + drift
        end
        ...
        if container.randomizePitch then
          local randomPitch = itemData.originalPitch + Utils.randomInRange(container.pitchRange.min, container.pitchRange.max)
        ...
        if container.randomizeVolume then
          local randomVolume = itemData.originalVolume * 10^(Utils.randomInRange(container.volumeRange.min, container.volumeRange.max) / 20)
        ...
        if container.randomizePan then
          local randomPan = itemData.originalPan + Utils.randomInRange(container.panRange.min, container.panRange.max) / 100
...
function Generation.generateTracks()
  ...
  for i, track in ipairs(globals.tracks) do
    ...
    for j, container in ipairs(track.containers) do
      ...
      Generation.placeItemsForContainer(container, containerTrack, xfadeshape)
    end
  end
...
-- Function to regenerate a single track
  ...
  if existingTrack then
    ...
    for j, container in ipairs(track.containers) do
      if j <= #containerTracks then
        Generation.placeItemsForContainer(container, containerTracks[j], xfadeshape)
      end
    end
  else
